...
const CustomerLogin: React.FC<CustomerLoginProps> = ({ onCancel, message }) => {
        ...
        <div className="flex min-h-screen items-center justify-center bg-background-dark p-4 animate-in fade-in zoom-in-95 duration-300">
            <div className="w-full max-w-md bg-[#111722] border border-white/10 rounded-2xl shadow-2xl p-8 flex flex-col gap-6 relative overflow-hidden">
                ...
                <div className="text-center">
                    ...
                    <h1 className="text-2xl font-black text-white uppercase tracking-wider mb-2">Meus Agendamentos</h1>
                    <p className="text-slate-500 text-sm">
                        {message || 'Faça login para acessar seus agendamentos.'}
                    </p>
                </div>
                ...
                <button
                    ...
                    disabled={isLoading}
                    className={`w-full h-14 bg-white hover:bg-gray-50 text-gray-700 font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-3 active:scale-[0.98] ${isLoading ? 'opacity-70 cursor-wait' : ''}`}
                >
                    {isLoading ? (
                ...
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-slate-600 text-xs uppercase tracking-wider">ou</span>
                    <div className="flex-1 h-px bg-white/10"></div>
                </div>
